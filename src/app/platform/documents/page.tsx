    ...
    <div className="p-6 min-h-screen">
      ...
      <div className="relative mb-5">
        ...
        <input value={query} onChange={e => setQuery(e.target.value)}
          ...
          className="w-full pl-9 pr-4 py-2.5 text-sm bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-lg outline-none focus:border-violet-500/60" />
      </div>

      <div className="grid grid-cols-2 gap-4">
        {/* List */}
        <div>
          <p className="text-xs font-semibold text-[var(--pios-muted)] uppercase tracking-wider mb-3">
              ...
              {filtered.map(doc => (
                <button key={doc.id} onClick={() => setSelected(doc)}
                  className={"w-full text-left p-3 rounded-lg border transition-colors " +
                    (selected?.id === doc.id ? "border-violet-500/60 bg-violet-500/5" : "border-[var(--pios-border)] bg-[var(--pios-surface)] hover:border-violet-500/30")}>
                  <div className="flex items-start gap-3">
                    <FileText className={"w-4 h-4 mt-0.5 flex-shrink-0 " + (doc.summary ? "text-violet-500" : "text-[var(--pios-muted)]")} />
        ...
        {/* Detail */}
        <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5 h-fit sticky top-6">
          {!selected ? (
            <div className="text-center py-10 text-[var(--pios-muted)] text-sm">
            ...
            <>
              ...
              {!selected.summary ? (
                ...
                <>
                  ...
                  <p className="text-sm text-[var(--pios-muted)] leading-relaxed mb-4 bg-violet-500/5 p-3 rounded-lg">{selected.summary}</p>
                  ...
                  <button onClick={() => analyse(selected)} disabled={analysing}
                    className="mt-4 flex items-center gap-1.5 text-xs text-[var(--pios-muted)] border border-[var(--pios-border)] rounded px-3 py-1.5 hover:text-foreground">
