  ...
  return (
    <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-4">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-xs text-[var(--pios-muted)] uppercase tracking-wide font-medium">{label}</span>
      </div>
      <div className="text-2xl font-semibold text-[var(--pios-text)]">{value}</div>
      {sub && <div className="text-xs text-[var(--pios-muted)] mt-1">{sub}</div>}
    </div>
  )
}
...
export default function ExecutivePage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-[var(--pios-muted)] text-sm animate-pulse">Loading Executive OS…</div>
    </div>
  )
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-2 gap-5">
          ...
          <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5">
            ...
            {okrs.length === 0 && (
              <div>
                <p className="text-xs text-[var(--pios-muted)]">No active OKRs. Add one to start tracking.</p>
          ...
          {/* Report Pack */}
          <div className="col-span-full flex items-center justify-between p-4 bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl">
            <div>
          ...
          {reportPack && (
            <div className="col-span-full bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5">
              ...
              <pre className="text-xs text-[var(--pios-text)]/80 leading-relaxed whitespace-pre-wrap font-sans">{reportPack}</pre>
            </div>
          )}
          {/* Open decisions */}
          <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5">
            ...
            {decisions.filter(d => d.status === 'open').length === 0 && <p className="text-xs text-[var(--pios-muted)]">No open decisions.</p>}
          </div>
          ...
          <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5">
            ...
            {stakeholders.filter(s => s.next_touchpoint && new Date(s.next_touchpoint) <= new Date()).slice(0,4).map(s => (
              <div key={s.id} className="flex items-center gap-3 py-2 border-b border-[var(--pios-border)] last:border-0">
                <div className={`text-xs font-semibold w-12 ${IMP_COLOR[s.importance]}`}>{s.importance.toUpperCase()}</div>
                <div>
                  <div className="text-sm text-[var(--pios-text)]">{s.name}</div>
                  <div className="text-xs text-[var(--pios-muted)]">{s.role} · {s.organisation}</div>
                </div>
              </div>
            ))}
            {stakeholders.filter(s => s.next_touchpoint && new Date(s.next_touchpoint) <= new Date()).length === 0 &&
              <p className="text-xs text-[var(--pios-muted)]">All stakeholders up to date.</p>}
          </div>
          ...
          <div className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-5">
            ...
            {principles.length === 0 && <p className="text-xs text-[var(--pios-muted)]">Add principles to guide your decisions.</p>}
          </div>
        </div>
      )}
      ...
      {activeTab === 'okrs' && (
        <div>
          ...
          <div className="space-y-4">
            {okrs.map(o => (
              <div key={o.id} className={`border rounded-xl p-5 ${HEALTH_BG[o.health] ?? 'bg-[var(--pios-surface)] border-[var(--pios-border)]'}`}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold text-[var(--pios-text)]">{o.title}</h3>
                  <div className="flex items-center gap-3">
                    <span className="text-xs text-[var(--pios-muted)]">{o.period}</span>
      ...
      {activeTab === 'decisions' && (
        <div>
          ...
          <div className="space-y-3">
            {decisions.map(d => (
              <div key={d.id} className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-4">
      ...
      {activeTab === 'stakeholders' && (
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {stakeholders.map(s => (
              <div key={s.id} className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl p-4">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <div className="text-sm font-semibold text-[var(--pios-text)]">{s.name}</div>
                    <div className="text-xs text-[var(--pios-muted)]">{s.role} · {s.organisation}</div>
                  </div>
            ...
            {stakeholders.length === 0 && (
              <div className="col-span-2 text-center py-16 text-[var(--pios-muted)]">
              ...
              {contracts.map((c: Record<string,any>) => (
                <div key={c.id} className="rounded-lg border p-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-[var(--pios-muted)]">{c.counterparty} · {c.contract_type}</p>
                  </div>
      ...
      {showOKRModal && (
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.6)] flex items-center justify-center z-50 p-4">
          <div className="bg-background border border-[var(--pios-border)] rounded-2xl p-6 w-full max-w-md">
            ...
            <label className="text-xs text-[var(--pios-muted)] uppercase tracking-wide block mb-1">Period</label>
            <input className={inp} value={okrForm.period} onChange={e => setOkrForm(f => ({...f, period: e.target.value}))} />
            <label className="text-xs text-[var(--pios-muted)] uppercase tracking-wide block mb-1">Description (optional)</label>
      ...
      {showDecisionModal && (
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.6)] flex items-center justify-center z-50 p-4">
          <div className="bg-background border border-[var(--pios-border)] rounded-2xl p-6 w-full max-w-md">
            ...
            <label className="text-xs text-[var(--pios-muted)] uppercase tracking-wide block mb-1">Context</label>
            ...
            <label className="text-xs text-[var(--pios-muted)] uppercase tracking-wide block mb-1">Framework</label>
      ...
      {showStakeModal && (
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.6)] flex items-center justify-center z-50 p-4">
          <div className="bg-background border border-[var(--pios-border)] rounded-2xl p-6 w-full max-w-md">
            ...
            <label className="text-xs text-[var(--pios-muted)] uppercase tracking-wide block mb-1">Importance</label>
