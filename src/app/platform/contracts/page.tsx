    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(c => (
            <div key={c.id} className="bg-[var(--pios-surface)] border border-[var(--pios-border)] rounded-xl overflow-hidden">
              ...
              {expanded === c.id && (c.key_terms || c.obligations || c.notes) && (
                <div className="px-4 pb-4 border-t border-[var(--pios-border)] pt-3 space-y-2">
                  {c.key_terms && <div><span className="text-xs font-semibold text-[var(--pios-muted)] uppercase tracking-wide">Key terms</span><p className="text-xs mt-1 text-foreground/80">{c.key_terms}</p></div>}
                  {c.obligations && <div><span className="text-xs font-semibold text-[var(--pios-muted)] uppercase tracking-wide">Obligations</span><p className="text-xs mt-1 text-foreground/80">{c.obligations}</p></div>}
                  {c.notes && <div><span className="text-xs font-semibold text-[var(--pios-muted)] uppercase tracking-wide">Notes</span><p className="text-xs mt-1 text-foreground/80">{c.notes}</p></div>}
                </div>
              )}
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-50 p-4 overflow-y-auto" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="bg-background border border-[var(--pios-border)] rounded-2xl p-6 w-full max-w-lg space-y-4 my-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2"><label className="text-xs text-[var(--pios-muted)] mb-1 block">Title *</label><input value={form.title} onChange={e => setForm(p => ({ ...p, title: e.target.value }))} className="pios-input w-full" placeholder="e.g. VeritasEdge SaaS Licence — Qiddiya" /></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Type</label><select value={form.contract_type} onChange={e => setForm(p => ({ ...p, contract_type: e.target.value }))} className="pios-input w-full">{['client','supplier','employment','nda','licence','partnership','lease','service','other'].map(t => <option key={t} value={t}>{t}</option>)}</select></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Status</label><select value={form.status} onChange={e => setForm(p => ({ ...p, status: e.target.value }))} className="pios-input w-full">{['active','draft','pending','expired','terminated','renewed'].map(s => <option key={s} value={s}>{s}</option>)}</select></div>
              <div className="col-span-2"><label className="text-xs text-[var(--pios-muted)] mb-1 block">Counterparty *</label><input value={form.counterparty} onChange={e => setForm(p => ({ ...p, counterparty: e.target.value }))} className="pios-input w-full" placeholder="Company or individual name" /></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Value</label><input type="number" value={form.value} onChange={e => setForm(p => ({ ...p, value: e.target.value }))} className="pios-input w-full" placeholder="0" /></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Currency</label><select value={form.currency} onChange={e => setForm(p => ({ ...p, currency: e.target.value }))} className="pios-input w-full">{['GBP','USD','EUR','AED','SAR'].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Start date</label><input type="date" value={form.start_date} onChange={e => setForm(p => ({ ...p, start_date: e.target.value }))} className="pios-input w-full" /></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">End date</label><input type="date" value={form.end_date} onChange={e => setForm(p => ({ ...p, end_date: e.target.value }))} className="pios-input w-full" /></div>
              <div><label className="text-xs text-[var(--pios-muted)] mb-1 block">Notice period (days)</label><input type="number" value={form.notice_period_days} onChange={e => setForm(p => ({ ...p, notice_period_days: e.target.value }))} className="pios-input w-full" placeholder="30" /></div>
              ...
              <div className="col-span-2"><label className="text-xs text-[var(--pios-muted)] mb-1 block">Key terms</label><textarea value={form.key_terms} onChange={e => setForm(p => ({ ...p, key_terms: e.target.value }))} rows={2} className="pios-input w-full resize-none" /></div>
              <div className="col-span-2"><label className="text-xs text-[var(--pios-muted)] mb-1 block">Obligations</label><textarea value={form.obligations} onChange={e => setForm(p => ({ ...p, obligations: e.target.value }))} rows={2} className="pios-input w-full resize-none" /></div>
            </div>
