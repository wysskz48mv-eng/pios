...
export default function TimeSovereigntyPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="text-muted-foreground text-sm animate-pulse">Loading Time Sovereignty Agent…</div>
    </div>
  )
    ...
    <div className="p-6 min-h-screen max-w-4xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Protect strategic time · audit calendar · ensure busyness never replaces strategy</p>
        </div>
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-3 mb-6">
          <div className="bg-card border border-border rounded-xl p-4">
            <div className="text-xs text-muted-foreground mb-1">Protected blocks</div>
            <div className="text-2xl font-semibold">{summary.protected_blocks}</div>
            <div className="text-xs text-muted-foreground">of {summary.total_blocks} total</div>
          </div>
          {(['strategic','deep_work','admin'] as const).map(t => (
            <div key={t} className="bg-card border border-border rounded-xl p-4">
              <div className="text-xs text-muted-foreground mb-1 capitalize">{t.replace('_',' ')}</div>
      ...
      {aiAudit && (
        <div className="mb-6 bg-cyan-500/5 border border-cyan-500/20 rounded-xl p-5">
          ...
          <div className="text-sm text-foreground/90 whitespace-pre-wrap leading-relaxed">{aiAudit}</div>
        </div>
      )}
              ...
              <div key={b.id} className="bg-card border border-border rounded-xl p-4 flex items-center gap-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-muted-foreground">{b.start_time} – {b.end_time}</span>
                    <span className="text-xs text-muted-foreground">{(b.day_of_week ?? []).map((d: number) => DAYS[d]).join(', ')}</span>
                  </div>
                  {b.notes && <p className="text-xs text-muted-foreground mt-1">{b.notes}</p>}
                </div>
                <button onClick={() => deleteBlock(b.id)} className="text-muted-foreground hover:text-red-400 transition-colors">
      ...
      {activeTab === 'log' && (
        <div className="max-w-md">
          <p className="text-sm text-muted-foreground mb-5">Log how you actually spent your time this week. This feeds your TSA™ audit history.</p>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div className="flex flex-col justify-end">
              <div className="text-xs text-muted-foreground mb-1">Strategic ratio</div>
              ...
              <div className="text-xs text-muted-foreground">target: 30–40%</div>
            </div>
          </div>
      ...
      {activeTab === 'audit' && (
        <div>
          {audits.length > 0 ? (
            <div className="space-y-3">
              {audits.map(a => (
                <div key={a.week_start} className="bg-card border border-border rounded-xl p-4">
      ...
      {showBlockModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4">
          <div className="bg-background border border-border rounded-2xl p-6 w-full max-w-md">
            ...
            <label className="text-xs text-muted-foreground block mb-1">Block type</label>
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input type="time" value={blockForm.start_time} onChange={e => setBlockForm(f=>({...f,start_time:e.target.value}))} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-foreground focus:outline-none" />
              </div>
              <div>
                <label className="text-xs text-muted-foreground block mb-1">End time</label>
                <input type="time" value={blockForm.end_time} onChange={e => setBlockForm(f=>({...f,end_time:e.target.value}))} className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/10 text-sm text-foreground focus:outline-none" />
              </div>
            </div>
            ...
            <div className="flex gap-1.5 mb-3 flex-wrap">
              {DAYS.map((d,i) => (
                <button key={d} type="button"
                  ...
                  className={`px-2 py-1 rounded text-xs font-medium transition-colors ${blockForm.day_of_week.includes(i) ? 'bg-violet-500 text-white' : 'bg-white/5 text-muted-foreground hover:bg-white/10'}`}>
                  {d}
                </button>
              ))}
            </div>
            ...
            <div className="flex gap-2">
              <button onClick={()=>setShowBlockModal(false)} className="flex-1 py-2 rounded-lg border border-border text-sm text-muted-foreground hover:bg-white/5">Cancel</button>
