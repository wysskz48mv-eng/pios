...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === 'ai_prioritise') {
      const { tasks } = body
      const system = `You are PIOS AI helping a senior executive and founder. Analyse the task list and return a prioritised order with reasoning. Analyse his task list and return a prioritised order with reasoning.
