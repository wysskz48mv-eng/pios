...
export function DomainCard({ domain, isSelected, onSelect, onUpdate }) {
  ...
  return <>
    <div onClick={onSelect} className={`p-6 rounded-lg border-2 cursor-pointer ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 bg-white'}`}>
      ...
      <button onClick={(e) => { e.stopPropagation(); setShowModal(true) }} className="mt-4 px-3 py-1 bg-gray-100 text-gray-700 rounded text-sm">Edit</button>
    </div>
